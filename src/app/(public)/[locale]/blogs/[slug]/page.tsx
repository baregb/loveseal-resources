...
export async function generateMetadata({ params }: PageParams): Promise<Metadata> {
  ...
  return generateContentMetadata(slug, locale, 'blog')
}

export default async function BlogsDetailPage({ params }: PageParams) {
  const { slug, locale } = await params
  ...
  const { item, attachments, signedPdfUrl, translationStatus, prefix, seriesItems } =
    await fetchContentDetail(slug, locale, 'blog')
