...
export async function generateMetadata({ params }: PageParams): Promise<Metadata> {
  ...
  return generateContentMetadata(slug, locale, 'sermon')
}

export default async function SermonNotesDetailPage({ params }: PageParams) {
  const { slug, locale } = await params
  ...
  const { item, attachments, signedPdfUrl, translationStatus, prefix, seriesItems, relatedItems } =
    await fetchContentDetail(slug, locale, 'sermon')
